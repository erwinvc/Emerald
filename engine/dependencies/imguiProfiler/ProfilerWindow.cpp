...
namespace ImGuiProfiler {
	...
	static void DrawProfilerTimeline(const ImVec2& size = ImVec2(0, 0)) {
		...
		if (ImGui::ItemAdd(timelineRect, timelineID)) {
			...
			if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && ImGui::IsMouseHoveringRect(timelineRect.Min, timelineRect.Max)) {
			...
			*/
			auto TrackHeader = [&](const char* pName, uint32 id) {
				...
				if (ImGui::ItemAdd(ImRect(trackTextCursor, trackTextCursor + ImVec2(caretSize, caretSize)), id)) {
					...
					pDraw->AddText(ImGui::GetItemRectMin() + ImVec2(2, 2), ImColor(style.BGTextColor), isOpen ? P_ICON_FA_CARET_DOWN : P_ICON_FA_CARET_RIGHT);
...

			{
				...
				pDraw->AddLine(ImVec2(timelineRect.Min.x, cursor.y),
					ImVec2(timelineRect.Max.x, cursor.y),
					ImColor(style.BGTextColor), 4);

				// Now draw the *new* GPU profiler track
// For convenience, get the ring buffer range from gCustomGPUProfiler
				auto range = gOpenGLProfiler.GetFrameRange();
				...
				bool isOpen = true; // or make it collapsible with the same "TrackHeader" logic
				{
					...
					pDraw->AddRectFilled(ImVec2(timelineRect.Min.x, cursor.y),
						ImVec2(timelineRect.Max.x, cursor.y + style.BarHeight),
						ImColor(0.0f, 0.0f, 0.0f, 0.3f));
					// Add text
					pDraw->AddText(ImVec2(timelineRect.Min.x + 5, cursor.y + 2),
						ImColor(style.BGTextColor), trackName);
				...
				for (uint32_t frameIndex = frameRange.Begin; frameIndex < frameRange.End; ++frameIndex) {
					...
					for (size_t i = 0; i < frameData.Events.size(); i++) {
						...
						if (hovered) {
							if (ImGui::BeginTooltip()) {
								float ms = TicksToMs * (float)(evt.TicksEnd - evt.TicksBegin);
								ImGui::Text("%s | %.3f ms", evt.pName, ms);
								ImGui::Text("Frame %d", frameIndex);
								// etc. if you have file/line info
								ImGui::EndTooltip();
							}
						}
					}
				...
				// After placing all events in this track, increment cursor:
				cursor.y += trackDepth * style.BarHeight;

				// line across
				pDraw->AddLine(ImVec2(timelineRect.Min.x, cursor.y),
					ImVec2(timelineRect.Max.x, cursor.y),
					ImColor(style.BGTextColor));
			...
			if (ImGui::IsWindowFocused()) {
				...
				if (zoomDelta != 0) {
					...
					logScale += zoomDelta;
					float newScale = ImClamp(expf(logScale), 0.1f, 500.0f);
	...
	void DrawProfilerHUD() {
		...
		ImGui::SameLine();
		if (ImGui::Button(P_ICON_FA_TIMES "##clearfilter"))
			context.SearchString[0] = 0;
		//ImGui::SameLine();
		//if (ImGui::Button(P_ICON_FA_PAINT_BRUSH "##styleeditor"))
		...
		if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
			!ImGui::IsAnyItemActive() && ImGui::IsKeyPressed(ImGuiKey_Space)) {
