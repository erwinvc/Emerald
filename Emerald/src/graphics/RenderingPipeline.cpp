...
void RenderingPipeline::Initialize(int maxLights, int lightQuality) {
    ...
    m_ssaoFBO = new FrameBuffer(1920, 1080, false, Color(0.3f, 0.0f, 0.0f, 1.0f));
    ...
    for (unsigned int i = 0; i < 16; i++) {
        Color noise(Math::RandomFloat(0.0f, 1.0f) * 2.0 - 1.0, Math::RandomFloat(0.0f, 1.0f) * 2.0 - 1.0, 0.0f, 1); // rotate around z-axis (in tangent space)
...
void RenderingPipeline::Render() {
	...
	case 5: m_ssaoFBO->GetTexture()->Bind(); break;
    }
    m_quad->Draw();
    ...
    if (ImGui::Begin("Emerald###Window", &op3en, ImVec2(576, 680), -1)) {
        ...
        if (ImGui::CollapsingHeader("HDR")) {
            ...
            if (ImGui::TreeNode("Framebuffers")) {
				...
				ImGui::SameLine();
				if (ImGui::ImageButton((void*)m_ssaoFBO->GetTexture()->GetHandle(), ImVec2(192, 108), ImVec2(0, 1), ImVec2(1, 0), 2)) selectedTexture = 5;
                ImGui::TreePop();
                ImGui::Separator();
            }
        }
