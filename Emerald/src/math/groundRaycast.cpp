...
glm::vec3 GroundRaycast::GetMousePosition() {
	return CalculateMouseRay();
}

glm::vec3 GroundRaycast::GetScreenPosition(glm::vec2 position) {
	position.x = (position.x * 2.0f) - 1.0f;
	position.y = -1.0f * ((position.y * 2.0f) - 1.0f);
	return CalculateScreenRay(glm::vec2(position));
}

glm::vec3 GroundRaycast::CalculateScreenRay(const glm::vec2& position) {
...
glm::vec3 GroundRaycast::CalculateMouseRay() {
	Camera* camera = GetCamera();
	float mouseX = GetMouse()->GetPosition().x;
	float mouseY = GetMouse()->GetPosition().y;
	glm::vec4& viewport = GetCamera()->GetViewport();
...
glm::vec3 GroundRaycast::GetGroundPosition(const glm::vec3& ray, float height) {
	glm::vec3& rayStart = GetCamera()->transform.m_position;
	glm::vec3& rayEnd = GetCamera()->transform.m_position + ray;
