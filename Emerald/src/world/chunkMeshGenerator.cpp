			...
			for (int x = 0; x < CHUNK_SIZE; x++) {
				...
				if (voxel > 0) {
					...
					if (MakeFace(voxel, chunk.GetBlock({ x - 1, y, z }))) {
						mesh.AddFace(LEFT_FACE, BlockSide::LEFT, voxelPosition, voxel);
					}

					// Right chunk face
					if (MakeFace(voxel, chunk.GetBlock({ x + 1, y, z }))) {
						mesh.AddFace(RIGHT_FACE, BlockSide::RIGHT, voxelPosition, voxel);
					}

					// Front chunk face
					if (MakeFace(voxel, chunk.GetBlock({ x, y, z + 1 }))) {
						mesh.AddFace(FRONT_FACE, BlockSide::FRONT, voxelPosition, voxel);
					}

					// Back chunk face
					if (MakeFace(voxel, chunk.GetBlock({ x, y, z - 1 }))) {
						mesh.AddFace(BACK_FACE, BlockSide::BACK, voxelPosition, voxel);
					}

					// Bottom chunk face
					if (MakeFace(voxel, chunk.GetBlock({ x, y - 1, z }))) {
						mesh.AddFace(BOTTOM_FACE, BlockSide::BOTTOM, voxelPosition, voxel);
					}

					// Top chunk face
					if (MakeFace(voxel, chunk.GetBlock({ x, y + 1, z }))) {
						mesh.AddFace(TOP_FACE, BlockSide::TOP, voxelPosition, voxel);
					}
				}
			}
		}
