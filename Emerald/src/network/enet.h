...
public:
	...
	template<typename T>
	void SendPacket(const T* data, int channel = 0, uint32 flags = 0) {
...
static void ConnectClientToServer(ENetHost* host, Connection& serverConnection, const char* ipAddress, int port) {
	...
	if (!connected) {
		LOG_ERROR("Failed to establish connection with the server");
	}
}